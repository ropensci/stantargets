tar_test("tar_stan_mcmc_rep_draws(compile = \"original\")", {
  skip_on_cran()
  ...
  expect_equal(nrow(out1), 160L)
  expect_equal(nrow(out2), 160L)
...
tar_test("tar_stan_mcmc_rep_draws(compile = \"copy\") custom variables", {
  skip_on_cran()
  skip_if_not_installed("dplyr")
