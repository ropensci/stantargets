targets::tar_test("tar_stan_vb_rep_summary(compile = \"original\")", {
  skip_on_cran()
  ...
  expect_equal(nrow(out1), 52L)
  expect_equal(nrow(out2), 52L)
...
targets::tar_test("tar_stan_vb_rep_summary(compile = \"copy\") custom", {
  skip_on_cran()
  skip_if_not_installed("dplyr")
  ...
  expect_true(all(out1$custom == 123L))
  expect_true(all(out2$custom == 123L))
