#' @title Multiple iterations of variational Bayes with summaries.
#' @export
...
#' @return `tar_stan_vb_rep_summary(name = x, ...)` returns a list
...
#'   * `x`: dynamic branching target with tidy data frames of VB summaries.
...
tar_stan_vb_rep_summary_run <- function(
  ...
  out <- purrr::map2_dfr(
    .x = data,
    .y = seeds,
    ~tar_stan_vb_rep_summary_run_rep(
      data = .x,
      seed = .y,
      model = model,
      refresh = refresh,
...
tar_stan_vb_rep_summary_run_rep <- function(
  data,
  seed,
  model,
  refresh,
