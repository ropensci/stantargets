#' @title Multiple optimization runs (long form).
#' @export
...
#' @return `tar_stan_mle_rep_summary(name = x, ...)` returns a list
...
tar_stan_mle_rep_summary_run <- function(
  ...
  out <- purrr::map2_dfr(
    .x = data,
    .y = seeds,
    ~tar_stan_mle_rep_summary_run_rep(
      data = .x,
      seed = .y,
      model = model,
      refresh = refresh,
...
tar_stan_mle_rep_summary_run_rep <- function(
  data,
  seed,
  model,
  refresh,
