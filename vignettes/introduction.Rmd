...
1. Use the fitted model object to extract posterior draws of parameters and store them in a tidy ata frame.
...
Above, `tar_stan_mcmc(example, ...)` only *defines* the pipeline. It does not actually run Stan, it declares the targets that will eventually run Stan. The specific targets are as follows.
