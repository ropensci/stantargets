...
* Add new arguments `variables_fit` and `inc_warmup_fit` to be passed to the `CmdStanFit` target in `tar_stan_mcmc()` and `tar_stan_gq()` instead of `variables` and `inc_warmup` (#56, @yizhang-yiz).
